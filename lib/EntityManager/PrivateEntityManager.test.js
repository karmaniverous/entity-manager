  ...
  describe('getIndexComponents', function () {
    const indexTestConfig = {
      entities: {
        foo: {
          indexes: {
            tree: ['entityPK', 'entitySK', 'treeSK'],
          },
          keys: {
            entityPK: {
              encode: ({ entityToken = 'foo', shardId }) =>
                `${entityToken}!${shardId ?? ''}`,
              decode: (value) =>
                value.match(/^(?<entityToken>.*)!(?<shardId>.*)$/)?.groups,
            },
            entitySK: {
              encode: ({ fooId }) => `fooId#${fooId ?? ''}`,
              decode: (value) =>
                value.match(/^fooId#(?<fooId>.*)$/)?.groups,
            },
            treeSK: {
              encode: ({ treeVal }) => `tree#${treeVal ?? ''}`,
              decode: (value) =>
                value.match(/^tree#(?<treeVal>.*)$/)?.groups,
            },
          },
        },
      },
    ...
    it('should return primary key components when indexToken is undefined', function () {
      const privateEntityManager = new PrivateEntityManager({
        config: indexTestConfig,
      });

      expect(
        privateEntityManager.getIndexComponents('foo', undefined)
      ).to.deep.equal(['entityPK', 'entitySK']);
    });

    it('should return primary key components when indexToken is null', function () {
      const privateEntityManager = new PrivateEntityManager({
        config: indexTestConfig,
      });

      expect(
        privateEntityManager.getIndexComponents('foo', null)
      ).to.deep.equal(['entityPK', 'entitySK']);
    });

    it('should return index components for a valid indexToken', function () {
      const privateEntityManager = new PrivateEntityManager({
        config: indexTestConfig,
      });

      expect(
        privateEntityManager.getIndexComponents('foo', 'tree')
      ).to.deep.equal(['entityPK', 'entitySK', 'treeSK']);
    });

    it('should throw for an invalid indexToken', function () {
      const privateEntityManager = new PrivateEntityManager({
        config: indexTestConfig,
      });

      expect(() =>
        privateEntityManager.getIndexComponents('foo', 'nonexistent')
      ).to.throw();
    });
  });
